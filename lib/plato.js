...
var coffeelint = require('./reporters/coffeelint');
  ...
  var runReports = function(files,done) {
    files.forEach(function(file) {
      ...
      if (options.recurse && fs.statSync(file).isDirectory()) {
        ...
        if (isCoffee) {
            coffeeSource = source;
            options.coffeeSource = coffeeSource;
            source = coffee.compile(coffeeSource);
        }
        ...
        _.each(reporters,function(reporter, name){
          ...
          try {
            if (name === 'jshint' && isCoffee) {
                report[name] = coffeelint.process(coffeeSource, flags[name], report.info);
            } else {
...
function writeFileReport(outdir, report, source, options) {
  ...
  var parsed = _.template(overviewSource, {
    source : util.escapeHTML((options.coffeeSource) ? options.coffeeSource : source),
    report : report
  });
